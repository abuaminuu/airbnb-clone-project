...
format: **entity: fields ---> relations**
...
Monitoring is the continuous observation of a system’s performance, availability, and health in real time. It helps detect issues early, such as high CPU usage, memory leaks, or slow response times, allowing developers to respond before users are affected. Tools like **Prometheus**, **Grafana**, or **Datadog** collect and visualize metrics such as request latency, error rates, and resource consumption, providing insights that support optimization and capacity planning.

### Logging

Logging involves recording detailed events or messages generated by applications and systems during operation. It provides a chronological trace of activities—such as user actions, system errors, and API requests—that developers can analyze when debugging or performing audits. Using structured formats (like JSON) and tools such as the **ELK Stack** (Elasticsearch, Logstash, Kibana) or **Fluentd**, teams can store, search, and analyze logs efficiently to pinpoint root causes of issues and ensure accountability.

### Feedback

Feedback is the process of using monitoring and logging data to drive action and continuous improvement. It closes the loop by enabling alerts, reports, or automated responses when specific thresholds or anomalies are detected. For example, an alert can be sent to the DevOps team when an error rate exceeds 5%, prompting quick intervention or triggering automated scaling to maintain performance. Effective feedback mechanisms turn raw operational data into meaningful insights that enhance reliability and resilience over time.
